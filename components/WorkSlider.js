        ...
        <SwiperSlide key={index}>
          <div className='grid grid-cols-2 grid-rows-2 gap-4 cursor-pointer'>
            {slide.images.map((image, imageIndex) => (
              <div key={imageIndex} className='group relative rounded-lg overflow-hidden flex items-center justify-center group key={index}'>
                <Image
                  src={image.path}
                  width={500}
                  height={500}
                ...
                group-hover:xl:-translate-y-20 transition-all duration-300'>
                  <div className='flex items-center gap-x-2 text-[13px] tracking-[0.2rem]'>
                    ...
                    {/*icon*/}
                    <div className='text-xl translate-y-[500%] group-hover:translate-y-0 '><BsArrowRight/> </div>
                    </div>
